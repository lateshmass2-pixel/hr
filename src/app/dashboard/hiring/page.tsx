        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#2a2a2a]">
                <div className="flex items-center gap-4">
                    <h2 className="text-2xl font-bold text-white tracking-tight">Hiring Pipeline</h2>
                    <Badge className="bg-violet-500/20 text-violet-400 border border-violet-500/30 font-medium">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-10 pr-4 py-2 border border-[#2a2a2a] rounded-lg bg-[#1a1a1a] text-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 transition-all w-64 placeholder-zinc-500"
                        />
                    </div>
                    <CreateJobDialog />
                    <UploadDialog />
    ...
    return (
        <div className="flex-1 min-w-[300px] flex flex-col gap-3 h-full max-h-[calc(100vh-240px)]">
            ...
            <div className="flex items-center justify-between px-2 mb-2">
                ...
                <span className="bg-[#2a2a2a] text-zinc-300 px-2 py-0.5 rounded-full text-[10px] font-semibold border border-[#333]">
                    {count}
                </span>
            </div>

            {/* Scrollable Card Area */}
            <div className="space-y-3 overflow-y-auto flex-1 pr-1 bg-[#1a1a1a]/50 rounded-xl p-3 border border-[#2a2a2a]">
                {isEmpty ? (
                    <div className="border-2 border-dashed border-[#333] rounded-xl h-32 flex items-center justify-center text-zinc-500 text-xs">
                        No candidates
                    </div>
                ) : (
                    children
...
function CandidateCard({ app, showScheduleButton, showDecisionButtons, isHired }: { app: any, showScheduleButton?: boolean, showDecisionButtons?: boolean, isHired?: boolean }) {
    ...
    return (
        <div className={`group bg-[#1a1a1a] p-4 rounded-xl border border-[#2a2a2a] shadow-lg hover:border-violet-500/50 transition-all duration-300 cursor-grab ${isHired
            ...
            <div className="flex items-start gap-3 mb-3">
                {/* Avatar */}
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-500 to-purple-400 text-white text-xs font-bold flex items-center justify-center shrink-0">
                    {initials}
                </div>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                        ...
                        <h4 className="font-semibold text-white text-sm truncate" title={app.candidate_name}>
            ...
            {hasScore && (
                <div className="mb-3">
                    <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[10px] text-zinc-500 font-medium uppercase tracking-wider">AI Match Score</span>
                    ...
                    <div className="h-1.5 bg-[#2a2a2a] rounded-full overflow-hidden">
                        <div
                            className={`h-full rounded-full transition-all ${isHighScore ? 'bg-emerald-500' : 'bg-violet-500'}`}
                            style={{ width: `${score}%` }}
                        />
                    </div>
                </div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
                {skills.map((skill, i) => (
                    <span key={i} className="bg-violet-500/20 text-violet-300 text-[10px] px-2 py-1 rounded font-medium border border-violet-500/20">
                        {skill}
                    </span>
                ))}
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-[#2a2a2a]">
                <div className="flex items-center gap-1.5 text-[10px] text-zinc-500 truncate flex-1">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {app.status === 'TEST_PENDING' && (
                        <Link href={`/assessment/${app.id}`} target="_blank">
                            <button className="p-1.5 hover:bg-[#2a2a2a] rounded-lg transition-colors text-zinc-500 hover:text-white" title="View Test">
                    ...
                    {app.resume_url && (
                        <Link href={app.resume_url} target="_blank">
                            <button className="p-1.5 hover:bg-[#2a2a2a] rounded-lg transition-colors text-zinc-500 hover:text-white" title="View Resume">
