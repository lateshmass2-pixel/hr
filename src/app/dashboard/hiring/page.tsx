...
import { Search, ExternalLink, Mail, CheckCircle2, Eye, Users, Sparkles } from "lucide-react"
...
export default async function HiringDashboard() {
    ...
    return (
        <div className="h-full flex flex-col">
            {/* Control Bar */}
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 pb-4 border-b border-[#e8e4e0] gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-[#1a1a1a] tracking-tight">Hiring Pipeline</h2>
                        <p className="text-[#6b6b6b] text-sm">{totalCandidates} active candidates</p>
                    </div>
                </div>
                <div className="flex items-center gap-3 flex-wrap">
                    {/* Help Bubble */}
                    <div className="hidden lg:flex items-center gap-2 bg-white rounded-full px-4 py-2 shadow-md border border-[#e8e4e0]">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-10 pr-4 py-2.5 border border-[#e8e4e0] rounded-full bg-white text-[#1a1a1a] text-sm focus:outline-none focus:ring-2 focus:ring-[#e07850]/20 focus:border-[#e07850] transition-all w-64 placeholder-[#a0a0a0] shadow-sm"
                        />
                    </div>
                    <CreateJobDialog />
                    <UploadDialog />
    ...
    return (
        <div className="flex-1 min-w-[300px] flex flex-col gap-3 h-full max-h-[calc(100vh-240px)]">
            ...
            <div className="flex items-center justify-between px-2 mb-2">
                <h3 className="text-xs font-semibold text-[#6b6b6b] tracking-wider uppercase">{title}</h3>
                <span className="bg-[#f5f3f0] text-[#6b6b6b] px-2.5 py-1 rounded-full text-[10px] font-semibold border border-[#e8e4e0]">
                    {count}
                </span>
            </div>

            {/* Scrollable Card Area */}
            <div className="space-y-3 overflow-y-auto flex-1 pr-1 bg-[#f5f3f0] rounded-2xl p-3 border border-[#e8e4e0]">
                {isEmpty ? (
                    <div className="border-2 border-dashed border-[#e8e4e0] rounded-xl h-32 flex items-center justify-center text-[#a0a0a0] text-xs bg-white">
                        No candidates
                    </div>
                ) : (
                    children
...
function CandidateCard({ app, showScheduleButton, showDecisionButtons, isHired }: { app: any, showScheduleButton?: boolean, showDecisionButtons?: boolean, isHired?: boolean }) {
    ...
    return (
        <div className={`group bg-white p-4 rounded-2xl border border-[#e8e4e0] shadow-md hover:shadow-lg hover:border-[#e07850]/30 transition-all duration-300 cursor-grab ${isHired
            ? 'border-[#e07850]/50 bg-[#e07850]/5'
            : ''
            }`}>
            ...
            <div className="flex items-start gap-3 mb-3">
                {/* Avatar */}
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#e07850] to-[#d45a3a] text-white text-xs font-bold flex items-center justify-center shrink-0 shadow-md">
                    {initials}
                </div>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                        ...
                        <h4 className="font-semibold text-[#1a1a1a] text-sm truncate" title={app.candidate_name}>
            ...
            {hasScore && (
                <div className="mb-3">
                    <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[10px] text-[#a0a0a0] font-medium uppercase tracking-wider">AI Match Score</span>
                    ...
                    <div className="h-1.5 bg-[#f5f3f0] rounded-full overflow-hidden">
                        <div
                            className={`h-full rounded-full transition-all ${isHighScore ? 'bg-emerald-500' : 'bg-[#e07850]'}`}
                            style={{ width: `${score}%` }}
                        />
                    </div>
                </div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
                {skills.map((skill, i) => (
                    <span key={i} className="bg-[#e07850]/10 text-[#e07850] text-[10px] px-2 py-1 rounded-full font-medium border border-[#e07850]/20">
                        {skill}
                    </span>
                ))}
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-[#e8e4e0]">
                <div className="flex items-center gap-1.5 text-[10px] text-[#a0a0a0] truncate flex-1">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {app.status === 'TEST_PENDING' && (
                        <Link href={`/assessment/${app.id}`} target="_blank">
                            <button className="p-1.5 hover:bg-[#f5f3f0] rounded-lg transition-colors text-[#a0a0a0] hover:text-[#1a1a1a]" title="View Test">
                    ...
                    {app.resume_url && (
                        <Link href={app.resume_url} target="_blank">
                            <button className="p-1.5 hover:bg-[#f5f3f0] rounded-lg transition-colors text-[#a0a0a0] hover:text-[#1a1a1a]" title="View Resume">
