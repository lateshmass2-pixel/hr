...
export function ProjectCard({ project, employees, users = [], onDelete }: ProjectCardProps) {
    const { tasks } = useHems()
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 max-w-[50%]">
                        {lead && (
                            <div className="flex items-center gap-2 border border-green-100 rounded-full px-2 py-1 bg-[#f8faf6] shrink-0">
                                ...
                                <span className="text-xs font-medium truncate max-w-[60px] text-green-900">{lead.full_name || lead.name}</span>
                            </div>
                        )}
                        {members.slice(0, 2).map(m => (
                            <div key={m.id} className="flex items-center gap-2 border border-green-100 rounded-full px-2 py-1 bg-white shrink-0">
                                ...
                                <span className="text-xs font-medium truncate max-w-[60px] text-green-800">{m.full_name || m.name}</span>
                            </div>
                        ))}
                    </div>
                ) : (
